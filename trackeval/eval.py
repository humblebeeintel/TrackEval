...
class Evaluator:
    ...
    def evaluate(self, dataset_list, metrics_list, show_progressbar=False):
        ...
        for dataset, dataset_name in tqdm.tqdm(zip(dataset_list, dataset_names)):
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    if dataset.should_classes_combine:
                        combined_cls_keys += ['cls_comb_cls_av',
                                              'cls_comb_det_av', 'all']
                    ...
                    if dataset.use_super_categories:
                        for cat, sub_cats in dataset.super_categories.items():
                            ...
                            for metric, metric_name in zip(metrics_list, metric_names):
                                ...
                                res['COMBINED_SEQ'][cat][metric_name] = metric.combine_classes_det_averaged(
                                    cat_res)
                    ...
                    # class_list + combined classes if calculated
                    for c_cls in res['COMBINED_SEQ'].keys():
                        summaries = []
                        details = []
                        ...
                        if config['OUTPUT_EMPTY_CLASSES'] or num_dets > 0:
                            for metric, metric_name in zip(metrics_list, metric_names):
                                ...
                                if c_cls in combined_cls_keys:
                                    table_res = {
                                        'COMBINED_SEQ': res['COMBINED_SEQ'][c_cls][metric_name]}
                                else:
                                ...
                                if config['OUTPUT_SUMMARY']:
                                    summaries.append(
                                        metric.summary_results(table_res))
                                if config['OUTPUT_DETAILED']:
                                    details.append(
                                        metric.detailed_results(table_res))
                            ...
                            if config['OUTPUT_DETAILED']:
                                utils.write_detailed_results(
                                    details, c_cls, output_fol)
