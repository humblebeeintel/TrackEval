...
class HOTA(_BaseMetric):
    ...
    def __init__(self, config=None):
        ...
        self.fields = self.float_array_fields + \
            self.integer_array_fields + self.float_fields
    ...
    def eval_sequence(self, data):
        ...
        if data['num_tracker_dets'] == 0:
            res['HOTA_FN'] = data['num_gt_dets'] * \
                np.ones((len(self.array_labels)), dtype=float)
            res['LocA'] = np.ones((len(self.array_labels)), dtype=float)
            res['LocA(0)'] = 1.0
            return res
        if data['num_gt_dets'] == 0:
            res['HOTA_FP'] = data['num_tracker_dets'] * \
                np.ones((len(self.array_labels)), dtype=float)
            res['LocA'] = np.ones((len(self.array_labels)), dtype=float)
            res['LocA(0)'] = 1.0
            return res
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            sim_iou_denom = similarity.sum(
                0)[np.newaxis, :] + similarity.sum(1)[:, np.newaxis] - similarity
            sim_iou = np.zeros_like(similarity)
            sim_iou_mask = sim_iou_denom > 0 + np.finfo('float').eps
            sim_iou[sim_iou_mask] = similarity[sim_iou_mask] / \
                sim_iou_denom[sim_iou_mask]
            potential_matches_count[gt_ids_t[:, np.newaxis],
        ...
        matches_counts = [np.zeros_like(potential_matches_count)
                          for _ in self.array_labels]
        from collections import defaultdict
        # frame_trackid_confs[alpha] = [(frame, track_id, conf)]
        frame_trackid_confs = defaultdict(list)
        ...
        for t, (gt_ids_t, tracker_ids_t, track_confs, ac_ids) in enumerate(zip(data['gt_ids'], data['tracker_ids'], data['tracker_confidences'], data['tracker_ids_original'])):
            ...
            for a, alpha in enumerate(self.array_labels):
                ...
                actually_matched_mask = similarity[match_rows,
                ...
                if num_matches > 0:
                    res['LocA'][a] += sum(similarity[alpha_match_rows,
                                          alpha_match_cols])
                    matches_counts[a][gt_ids_t[alpha_match_rows],
                                      tracker_ids_t[alpha_match_cols]] += 1
        ...
        for a, alpha in enumerate(self.array_labels):
            ...
            ass_a = matches_count / \
                np.maximum(1, gt_id_count + tracker_id_count - matches_count)
            res['AssA'][a] = np.sum(matches_count * ass_a) / \
                np.maximum(1, res['HOTA_TP'][a])
            ass_re = matches_count / np.maximum(1, gt_id_count)
            res['AssRe'][a] = np.sum(
                matches_count * ass_re) / np.maximum(1, res['HOTA_TP'][a])
            ass_pr = matches_count / np.maximum(1, tracker_id_count)
            res['AssPr'][a] = np.sum(
                matches_count * ass_pr) / np.maximum(1, res['HOTA_TP'][a])

        # Calculate final scores
        res['LocA'] = np.maximum(1e-10, res['LocA']) / \
            np.maximum(1e-10, res['HOTA_TP'])
        res = self._compute_final_fields(res)
        res['frame_trackid_confs'] = frame_trackid_confs
        return res
    ...
    def combine_sequences(self, all_res):
        ...
        res['LocA'] = np.maximum(1e-10, loca_weighted_sum) / \
            np.maximum(1e-10, res['HOTA_TP'])
        res = self._compute_final_fields(res)
        return res
    ...
    def combine_classes_class_averaged(self, all_res, ignore_empty_classes=False):
        ...
        for field in self.float_fields + self.float_array_fields:
            if ignore_empty_classes:
                ...
                res[field] = np.mean([v[field]
                                     for v in all_res.values()], axis=0)
        return res
    ...
    def _compute_final_fields(res):
        ...
        res['DetRe'] = res['HOTA_TP'] / \
            np.maximum(1, res['HOTA_TP'] + res['HOTA_FN'])
        res['DetPr'] = res['HOTA_TP'] / \
            np.maximum(1, res['HOTA_TP'] + res['HOTA_FP'])
        res['DetA'] = res['HOTA_TP'] / \
            np.maximum(1, res['HOTA_TP'] + res['HOTA_FN'] + res['HOTA_FP'])
    ...
    def plot_single_tracker_results(self, table_res, tracker, cls, output_folder):
        ...
        for name in self.float_array_fields:
            legend += [name +
                       ' (' + str(np.round(np.mean(res[name]), 2)) + ')']
